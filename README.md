...
A `.shard` is a verifiable ZIP archive. Inside: a **soul** (personality,
stats, appearance), a **shell** (body, anatomy), a **mind** (tiered memory),
and uniquely, a **neural substrate** that keeps ticking whether anyone is watching or not.
This is the first step toward persistent, living AI entities.
...
> of typed JSON pillars (`soulshard.json`, `shellshard.json`,
> `mindshard.json`, `neuronshard.json`) with a manifest carrying per-file
> SHA-256 digests, not a markdown file, a system-prompt fragment, or a
> character-card schema. The format, its pillars, and the Neuronshard
...
> in pure Python. v1.0 spec + v0.1 reference implementation below, skip
> to [Quickstart](#quickstart) if you want to run something now.
...
is... who they are, what they remember, how they feel, how they react...
in a single file you can share, copy, edit, or hand to a friend. Like
a PDF, but for personalities.
...
The format is open. The reference tools to load, verify, and run a
shard are open. **The characters and applications you build with it
are yours.** A graphical shard maker is on the roadmap; today you can
author a shard by hand or with the reference library in this repository.
...
normative (spec §3.4 `vitality_system`); the implication is the point.
...
constant attention, and nothing stops it from being practical. A well-
tended shard can hold context across months, track goals and drives
(v1.1), run its neural substrate in the background on modest hardware,
and bring a mind of its own to problems you hand it.

---

## Status

This repository is the **v1.0 public release** of the format spec plus a
**v0.1 reference implementation**.

| Pillar        | v1.0 status                                    |
|---------------|------------------------------------------------|
| Manifest      | **Normative** — SHA-256 verified, spec-frozen  |
| Soulshard     | **Normative** — full schema in `SHARDCORE_Spec_v1.0.md` |
| Shellshard    | **Normative** — anatomy, image paths, state    |
| Mindshard     | **Normative** — tiered memory (v2.1)           |
| Neuronshard   | **Normative** — LIF dynamics, byte-identical round-trip |
| Driveshard    | *Experimental* — schema sketched, engine in v1.1 |
| Skills folder (read-only) | *Experimental* — retrieval-gated abilities, schema in v1.1 |
| Skills folder (learned)   | *Experimental* — write-back / proficiency, v1.2 |
| Dream cycles  | *Experimental* — memory consolidation, v1.1    |

Normative sections are binding on conformant runtimes. Experimental
sections may change before v1.1. See [`SHARDCORE_Spec_v1.0.md`](SHARDCORE_Spec_v1.0.md)
...
# Validate a bundle against the v1.0 spec
python -m shardcore verify examples/minimal.shard

# Tick the neural substrate of a bundle for 200 steps and save resulting state
python -m shardcore neuron examples/standard.shard --ticks 200 --save state.json
```

`verify` checks that every file listed in the manifest matches its declared
SHA-256 digest and that required pillars are present. `neuron` builds a
spiking network from the bundle's soul, runs it, and optionally persists
the runtime state back out as a `neuronshard.json`.

Both commands exit 0 on success, non-zero on any spec violation.
...
```
shardcore/         Reference library, pure numpy, no LLM required
  verify.py        Manifest + SHA-256 + pillar validator
  neuron.py        LIF tick engine, topology builder, neuronshard I/O
  __main__.py      python -m shardcore dispatcher

tests/
  unit/            Reference behavior tests (10 tests, pytest)
  conformance/     Format conformance suite (bundles runtimes must load/reject)

examples/          Reference bundles (minimal + standard)
docs/              Design notes for experimental pillars
...
- [Full specification](SHARDCORE_Spec_v1.0.md)
- [Roadmap](ROADMAP.md)
...
- **Specification** (`SHARDCORE_Spec_v1.0.md` and any future spec
